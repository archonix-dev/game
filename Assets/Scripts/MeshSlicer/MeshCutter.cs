...
public class MeshCutter : MonoSingleton<MeshCutter>
{
    ...
    public GameObject Cut(GameObject cutObject, Vector3 cutPoint, Vector3 cutDirection, bool enableGravityForNewMesh = true, bool canNewMeshShatter = false, Transform parentForFragments = null, float destroyAfter = 0f)
    {
        ...
        // ОПТИМИЗАЦИЯ: Используем CapsuleCollider вместо тяжелого MeshCollider
        MeshCollider oldMeshCollider = cutObject.GetComponent<MeshCollider>();
        if (oldMeshCollider != null)
        {
            Destroy(oldMeshCollider);
        }
        
        CapsuleCollider capsule = cutObject.AddComponent<CapsuleCollider>();
        Bounds bounds = leftMeshFinal.bounds;
        capsule.center = bounds.center;
        capsule.radius = Mathf.Max(bounds.extents.x, bounds.extents.z) * 0.8f;
        ...
        GameObject offcutObject = new GameObject("MeshFragment");
        ...
        if (parentForFragments != null)
        {
            offcutObject.transform.SetParent(parentForFragments);
        }
        ...
        // ОПТИМИЗАЦИЯ: Используем CapsuleCollider вместо тяжелого MeshCollider
        CapsuleCollider offcutCapsule = offcutObject.AddComponent<CapsuleCollider>();
        Bounds rightBounds = finishedRightMesh.bounds;
        offcutCapsule.center = rightBounds.center;
        offcutCapsule.radius = Mathf.Max(rightBounds.extents.x, rightBounds.extents.z) * 0.8f;
        offcutCapsule.height = rightBounds.size.y;
        offcutCapsule.direction = 1; // Y-axis
        
        // Уничтожаем осколок через указанное время
        if (destroyAfter > 0f)
        {
            Destroy(offcutObject, destroyAfter);
        }
